...
- Fixed dropping an item while standing on a **moving contraption** launching it through the deck or far off into the world, stalling the server for tens of seconds per tick, and spamming `Enormous local sub-level collision bounds, quitting.` into the log. The 1.2.0 code handed the item the contraption's velocity at spawn, asking Sable for it at a **world** position where a sub-level-local one is required — so the tangential term scaled with the offset between the contraption's internal coordinates and the world's — around 20 million blocks — instead of the item's radius on the deck, inflating the velocity by orders of magnitude. That offset is there wherever the contraption is, which is why the bug did not need you to be far from spawn. Worst on levitating/drag-tuned rigs, which always carry a little angular velocity; harmless on parked ones, which is why it hid for two releases.
